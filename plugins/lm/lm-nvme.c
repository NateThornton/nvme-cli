...
{
	const char *desc = "Track Send command used to manage the tracking of information by a controller";
	...
	OPT_ARGS(opts) = {
		OPT_BYTE("select",    's', &cfg.select, select),
	...
	if (cfg.select != TRACK_SEND_SELECT_LOG_USER_DATA_CHANGES) {
		nvme_show_error("Unsupported select option %d (%s)", cfg.select,  ARGSTR(lm_track_send_select_argstr, cfg.select));
		return -EINVAL;
	}
	...
	if (cfg.start && cfg.stop) {
		...
		return -EINVAL;
	} else if (cfg.select == TRACK_SEND_SELECT_LOG_USER_DATA_CHANGES) {
		if (cfg.start)
	...
	struct nvme_passthru_cmd cmd = {
		...
		.cdw10 = (cfg.select) | (cfg.mos) << 16,
		.cdw11 = cfg.cdqid,
	};

	err = nvme_submit_admin_passthru(dev_fd(dev), &cmd, NULL);
	if (!err)
		printf("Track Send (%s) Successful\n", ARGSTR(lm_track_send_select_argstr, cfg.select));
	else if (err > 0)
		nvme_show_status(err);
	else
...
static void json_controller_state_data(struct controller_state_data *data,
				       size_t len)
{
				       ...
				       enum nvme_print_flags flags)
{
	...
	if (sizeof(struct nvme_controller_state_header) <= len) {
		...
		for (int i = 0; i < niocq; ++i) {
			...
			if (human) {
				printf("  [31:16] : 0x%x I/O Completion Queue Interrupt Vector (IOCQIV)\n",
				       attrs >> 16);
